...
class Network {
 public:
    ...
    void SGD(std::vector<csv::dataset*>*, std::vector<csv::dataset*>*,
            size_t, size_t, double, double);

    /* Updates given a mini batch of datasets */
    void update_mini_batch(std::vector<csv::dataset*>*, double learning_rate, double lambda, int training_data_size);
    ...
    std::pair<std::vector<Matrix>, std::vector<Matrix> >* backprop(csv::dataset* dataset);
    ...
    int evaluate(std::vector<csv::dataset*>* datasets);
