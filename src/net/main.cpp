...
// TODO
// Read CSV file to get number of rows
...
/*  if ((args.size() - PARAM_SIZE) % 2 == 1) {
    ...
    std::vector<csv::dataset*> train_cases = csv::load_test_cases(args[0]);
    std::vector<csv::dataset*> test_cases;
    if (has_test) {
        test_cases = csv::load_test_cases(args[0]);
    }
