...
Matrix Matrix::operator*(const double_v& vec) const {
    ...
    if (this->cols == vec.size()) {
        ...
        for (size_t j = 0; j < this->cols; ++j) {
            for (size_t i = 0; i < this->rows; ++i) {
                result.set_index(i, j, this->index(i, j) * vec[j]);
            }
        }
        ...
        for (size_t i = 0; i < this->rows; ++i) {
            for (size_t j = 0; j < this->cols; ++j) {
                result.set_index(i, j, this->index(i, j) * vec[i]);
            }
        }
    } else {
        std::stringstream err;
...
double Matrix::index(size_t row, size_t col) const {
    /*if (row >= this->rows || cols >= this->cols) {
        std::stringstream err;
        err << "Cannot index " << row << "," << col << " of matrix; out of bounds";
        throw std::runtime_error(err.str());
    }*/
