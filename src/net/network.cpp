...
Network::Network(std::string filename) {
    ...
    this->sizes.push_back(this->weights.at(0)->cols);
    for (Matrix* m : this->weights) {
        this->sizes.push_back(m->rows);
    }
    infile.close();
}
...
void Network::SGD(std::vector<csv::dataset*>* training_data, std::vector<csv::dataset*>* test_data,
    ...
    for (size_t j = 0; j < num_epochs; ++j) {
        ...
        time_point begin = timing::now();
        
        std::random_shuffle(training_data->begin(), training_data->end());
        /* Vector of all minibatches */
        std::vector<std::vector<csv::dataset*> > mini_batches(num_mini_batches,
            std::vector<csv::dataset*>(mini_batch_size));
        ...
        std::cout << "took " << timing::diff(begin,end) << " ms";
    }
}

/* Updates given a mini batch of datasets */
void Network::update_mini_batch(std::vector<csv::dataset*>* mini_batch,
    ...
    for (size_t i = 0; i < mini_batch->size(); ++i) {
        csv::dataset* dataset = mini_batch->at(i);
...
std::pair<std::vector<Matrix>, std::vector<Matrix> >* Network::backprop(csv::dataset* dataset) {
...
int Network::evaluate(std::vector<csv::dataset*>* datasets) {
    size_t num_worked = 0;
    for (size_t i = 0; i < datasets->size(); ++i) {
        csv::dataset* dataset = (*datasets)[i];
        double_v inputs;
